...
if (TerminusI18n.language === "zh") {
    man_pages = TerminusI18n.manPages;
}
    ...
    $('#term').terminal(function(input, term) {
        ...
        if( current_room.commands.indexOf(command) > -1 ){ //Could use current_room.hasOwnProperty(command)
            ...
            if (args.length > 0 && args[0].indexOf("/") > 0){
                ...
                for (var i = 0; i < rooms_in_order.length; i++){
                    ...
                    if ((command === "cd" || command === "ls") && cur_room_to_test === false){
                        TerminusI18n.echo(term, "That is not reachable from here.");
                        exec = false;
                    }
                }
            ...
            if (exec){
                ...
                if (text_to_display){
                    TerminusI18n.echo(term, text_to_display);
                }
                if (command in current_room.cmd_text){
                    TerminusI18n.echo(term, current_room.cmd_text[command]);
                }
            }
        }
        else{
            TerminusI18n.echo(term, "Command '"+command+"' not found in room '"+current_room.room_name+"'");
        }
        ...
        greetings: TerminusI18n.greetings,
